...
if($('#portfolio-items').length>0){var container=document.querySelector('#portfolio-items');var msnry=new Masonry(container,{itemSelector:'.portfolio-item',columnWidth:'.portfolio-sizer',percentPosition:true});}
if($('.portfolio-sidebar.sidebar-list').length>0){var $sidebar=$(".portfolio-sidebar.sidebar-list"),$window=$(window),$listHeight=$(".portfolio-gallery-list").height(),offset=$sidebar.offset(),topPadding=0;$sidebar.css('width',$sidebar.width());$window.scroll(function(){if($window.scrollTop()>offset.top-120){$sidebar.stop().addClass('fixed-sidebar');if($window.scrollTop()>offset.top+$listHeight-$sidebar.height()-100){$sidebar.addClass('absolute-sidebar');}
else{$sidebar.removeClass('absolute-sidebar');}}else
{$sidebar.stop().removeClass('fixed-sidebar');}});}});$(document).ready(function(){if($('#single-page').length>0){$('body').addClass('single-post');}
...
$('.navbar-nav > li.one-page-link > a:first-child, .footer_widget .menu li.one-page-link > a:first-child').each(function(){var href=$(this).attr("href");var link=href.replace(/\/$/,'');if($("body").hasClass("home")){var anchor=link.substr(link.lastIndexOf('/')+1);if(anchor.indexOf("?page_id")>=0){anchor='';}
$(this).attr("href",'#'+anchor);}else{if(!$(this).hasClass("first")){var pos=link.lastIndexOf('/');link=link.substring(0,pos)+'#'+link.substring(pos+1);$(this).attr("href",link);}}});$("<span class='mobile-dropdown'></span>").appendTo($(".navbar-nav .menu-item-has-children"));$(".navbar-nav .menu-item-has-children .mobile-dropdown").click(function(){$(this).closest(".menu-item-has-children").toggleClass('mobile-visible');});$('.navbar-collapse ul li a, .navbar-collapse .modal-menu-item.scroll-section').click(function(){$('.navbar-toggle:visible').click();});$(function(){$('.navbar-nav .one-page-link a, .footer_widget .menu li a, .scroll-section, .demo-button').bind('click',function(event){if($("body").hasClass("home")){var anchor=$(this);$('html, body').stop().animate({scrollTop:$(anchor.attr('href')).offset().top-68},1500,'easeInOutExpo');event.preventDefault();}});});$(function(){$('.portfolio-demo-button, .kd-calltoaction.smooth-scroll .tt_button').bind('click',function(event){var anchor=$(this);$('html, body').stop().animate({scrollTop:$(anchor.attr('href')).offset().top-250},1500,'easeInOutExpo');event.preventDefault();});});$('body').scrollspy({offset:69,target:'.navbar-fixed-top'});$(".header-search-bar .toggle-search").click(function(){$(".header-search-bar .header-search-container").toggleClass('active');$(this).toggleClass('fa-times');});if($('.wpglobus-selector-link').length>0){$('.wpglobus-selector-link .dropdown-menu').addClass('sub-menu');}
$('.slider-scroll-down a').click(function(event){$('html, body').animate({scrollTop:$("#wrapper .section").offset().top-68},1500,'easeInOutExpo');event.preventDefault();});$(window).bind("scroll",function(){if($(window).scrollTop()>$(window).height()){$('.back-to-top').addClass('active');}
if($(window).scrollTop()<$(window).height()){$('.back-to-top').removeClass('active');}});$('.back-to-top').click(function(event){event.preventDefault();$('html, body').animate({scrollTop:0},1000);return false;});});if($(".owlslider-portfolio").length){$(".owlslider-portfolio").owlCarousel({navigation:false,pagination:true,singleItem:true,autoPlay:false,animateOut:'fadeOut',});}
