...
if($(".features-tabs li.tab").length){$('.features-tabs').easytabs({updateHash:false,animationSpeed:'fast',transitionIn:'fadeIn'});}
$('.featured_content_child').on('mouseenter',function(){$('.featured_content_child').removeClass('active-elem');$(this).addClass('active-elem');});if($(".app-gallery .ag-slider").length){$(".app-gallery .ag-slider").owlCarousel({navigation:false,pagination:true,autoPlay:false,items:1,});}});jQuery(window).load(function(){if(jQuery('.mg-gallery').length>0){jQuery('.mg-gallery').each(function(){var msnry=new Masonry(this,{itemSelector:'.mg-single-img',columnWidth:'.mg-sizer',percentPosition:true,gutter:5});});var classes='.vc_tta-tabs-list .vc_tta-tab,'+'.vc_pagination .vc_pagination-item';jQuery('body').on('click',classes,function(){setInterval(function(){var reloadMasonry=jQuery('.vc_active .mg-gallery').masonry({itemSelector:'.mg-single-img',columnWidth:'.mg-sizer',percentPosition:true,gutter:5});reloadMasonry.masonry("reloadItems");reloadMasonry.masonry('layout');},200);});}
